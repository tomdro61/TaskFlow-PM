// renderer/events.js — Event binding, keyboard navigation, shortcuts
...
export function selectPrevTask() {
  const tasks = this.getVisibleTasks();
  if (tasks.length === 0) return;
...
export function highlightSelectedTask(tasks) {
...
export function selectTask(taskId, element) {
...
export function toggleSelectedTaskComplete() {
...
export function openSelectedTaskDetail() {
...
export function setSelectedTaskPriority(priority) {
  if (!this.selectedTaskId) return;
  this.updateTask(this.selectedTaskId, { priority });
  this.render();
}

export function toggleSelectedTaskToday() {
  if (!this.selectedTaskId) return;
  this.openSnoozePopup(this.selectedTaskId);
}

export function toggleSelectedTaskClaude() {
  if (!this.selectedTaskId) return;
...
export function toggleSelectedTaskSubtasks() {
  if (!this.selectedTaskId) return;
...
export function addSelectedTaskToActive() {
  if (!this.selectedTaskId) return;
  if (this.todayView.workingOnTaskIds.includes(this.selectedTaskId)) {
    this.removeActiveTask(this.selectedTaskId);
...
export function bindColorPicker(pickerId, inputId) {
