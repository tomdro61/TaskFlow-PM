// renderer/inbox-view.js — Inbox rendering, AI prompts
...
export function bindInboxEvents() {
    const today = this.getLocalDateString();
...
export function processInboxPrompt() {
    const inbox = this.data.projects.find(p => p.isInbox || p.id === 'inbox');
    const tasks = inbox ? inbox.tasks.filter(t => t.status !== 'done') : [];
...
export function impactReviewPrompt() {
